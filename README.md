...
2. **Engineering acceleration:** GPT-5.6 implemented the dependency-free audit engine, seeded simulator, CLI, HTTP/Vercel adapters, browser interface, and GitHub Action. Each executable adapter imports `src/audit.js`; the browser calls that engine through the HTTP adapter. No surface reimplements verdict logic.
